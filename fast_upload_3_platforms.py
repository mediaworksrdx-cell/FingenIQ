...
archive_path = r'C:\Users\daarv\.gemini\antigravity\scratch\fast_sync_3.tar.gz'

print('===========================================================')
print(' Fast Sync for 3 Platforms from C:\\Users\\daarv\\.gemini\\antigravity\\scratch:')
print('   1. FinGenIQ')
...
def exclude_filter(tarinfo):
    name = tarinfo.name
    # Exclude system binaries, huge non-code folders
    excluded = ['node_modules', 'venv', '.git', '__pycache__', 'llama.cpp', 'ios-app', 'chroma_db', '.next/cache']
    for exc in excluded:
        if f'/{exc}/' in name or name.endswith(f'/{exc}') or name.startswith(f'{exc}/') or name == exc:
            return None
    return tarinfo

print('[1/4] Archiving the 3 platforms...')
...
print(f'-> Archive Size: {size_mb:.2f} MB (Optimized!)')

print('[2/4] Uploading to server...')
subprocess.run(['scp'] + SSH_OPTS + [archive_path, f'{HOST}:/home/mediaworksr/fast_sync_3.tar.gz'], check=True)
print('-> Upload finished!')

print('[3/4] Extracting and restarting PM2...')
remote_script = (
    "cd /home/mediaworksr\n"
    "tar -xzf fast_sync_3.tar.gz\n"
    "rm -f fast_sync_3.tar.gz\n"
...
print('[4/4] ALL 3 PLATFORMS UPDATED AND RESTARTED CLEANLY!')
